...
import static com.depromeet.breadmapbackend.domain.notice.QNotice.notice;
...
public class NoticeQueryRepository {
    ...
    private BooleanExpression dateTimeCondition(NoticeDayType type, Long lastId, int page) {
        ...
        if (type.equals(NoticeDayType.TODAY)){
            ...
            booleanExpression = notice.createdAt.before(LocalDateTime.now().with(LocalTime.MAX).with(TemporalAdjusters.previous(DayOfWeek.SUNDAY)));
        }
        else throw new DaedongException(DaedongStatus.NOTICE_DAY_TYPE_EXCEPTION);

        if (lastId == null && page == 0) return booleanExpression;
        else if (lastId != null && page != 0) return booleanExpression.and(notice.id.lt(lastId));
        else throw new DaedongException(DaedongStatus.NOTICE_PAGE_EXCEPTION);
    }
}
