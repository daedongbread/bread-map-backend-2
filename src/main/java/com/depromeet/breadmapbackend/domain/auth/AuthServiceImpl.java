...
public class AuthServiceImpl implements AuthService {
	...
	public JwtToken login(LoginRequest request) {
		final OICDUserInfoTemp oicdUserInfo =
			oidcVerifyProcessor.verifyIdToken(request.getType(), request.getIdToken());

		User user = userRepository.findByOAuthId(oicdUserInfo.getOAuthId())
			.orElseThrow(() -> new DaedongException(DaedongStatus.USER_NOT_FOUND));

		if (user.getUserInfo().getEmail() == null)
			user.getUserInfo().updateEmail(oicdUserInfo.getEmail());
		if (user.getIsBlock())
			throw new DaedongException(DaedongStatus.BLOCK_USER);

		return createNewToken(oicdUserInfo.getOAuthId(), RoleType.USER);
	}
	...
	public JwtToken register(RegisterRequest request) {
		...
		if (!redisTokenUtils.isRejoinPossible(oicdUserInfo.getOAuthId()))
			throw new DaedongException(DaedongStatus.REJOIN_RESTRICT);

		if (userRepository.findByOAuthId(oicdUserInfo.getOAuthId()).isEmpty()) {
			createUser(oicdUserInfo, request.getIsMarketingInfoReceptionAgreed());
			return createNewToken(oicdUserInfo.getOAuthId(), RoleType.USER);
		} else
	...
	private void createUser(OICDUserInfoTemp oicdUserInfo, Boolean isMarketingInfoReceptionAgreed) {
		User user = User.builder()
			.oAuthInfo(OAuthInfo.builder()
				.oAuthType(oicdUserInfo.getOAuthType())
				.oAuthId(oicdUserInfo.getOAuthId()).build())
			.userInfo(UserInfo.builder()
				.nickName(createNickName())
				.email(oicdUserInfo.getEmail())
