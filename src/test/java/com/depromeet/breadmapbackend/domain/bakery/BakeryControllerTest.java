...
class BakeryControllerTest extends ControllerTest {
	...
	void getBakeryList() throws Exception {
		mockMvc.perform(
				get("/v1/bakeries?sortBy=distance&filterBy=false&latitude=37.560992&longitude=127.044174&latitudeDelta=0.01&longitudeDelta=0.02")
	...
	@Test
	void getNewBakery() throws Exception {
		mockMvc.perform(get("/v1/bakeries/new")
			...
			.andDo(document("v1/bakery/find/new",
				...
				responseFields(
					...
					fieldWithPath("data.[].name").description("빵집 이름"),
					fieldWithPath("data.[].pioneerId").description("빵집 개척자 id"),
					fieldWithPath("data.[].pioneerNickname").description("빵집 개척자 닉네임"),
					fieldWithPath("data.[].pioneerProfileImage").description("빵집 개척자 프로필 사진"),
					fieldWithPath("data.[].shortAddress").description("빵집 단축 주소"),
					fieldWithPath("data.[].content").description("빵집 추천글"),
					fieldWithPath("data.[].isFlagged").description("빵집 깃발 추가 여부"),
					fieldWithPath("data.[].isFollowed").description("빵집 개척차 팔로우 여부")
				)
			))
			.andExpect(status().isOk());
	}
