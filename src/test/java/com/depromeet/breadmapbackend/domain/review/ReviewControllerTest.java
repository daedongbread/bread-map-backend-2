...
class ReviewControllerTest extends ControllerTest {
		...
		mockMvc.perform(get("/v1/reviews/bakeries/{bakeryId}?sortBy=high&page=0", bakery.getId())
			...
			.andDo(document("v1/review/get/bakery",
				...
				responseFields(
					...
					fieldWithPath("data.contents.[].reviewInfo.productRatingList.[].productName").description(
						"리뷰 상품 이름"),
	...
	void getProductReviewList() throws Exception {
		mockMvc.perform(get("/v1/reviews/bakeries/{bakeryId}/products/{productId}?sortBy=low&page=0", bakery.getId(),
				product1.getId())
				.header("Authorization", "Bearer " + token.getAccessToken()))
			.andDo(print())
		...
		mockMvc.perform(post("/v1/reviews/bakeries/{bakeryId}", bakery.getId())
			...
			.andDo(document("v1/review/add",
				...
				requestFields(
					...
					fieldWithPath("noExistProductRatingRequestList.[].productType").description(
						"상품 타입 (BREAD, BEVERAGE, ETC)"),
